...
export const createPost =
    ...
    async (dispatch) => {
        try {
            ...
            const res = await axios.post('http://localhost:5000/api/v1/post/create', data);
...
export const updatePost =
    ...
    async (dispatch) => {
        try {
            dispatch({ type: NOTIFY_TYPES.NOTIFY, payload: { isLoading: true } });
            const res = await axios.patch(`http://localhost:5000/api/v1/post/update/${idPost}`, data);
...
export const deletePost = (idPost) => async (dispatch) => {
    try {
        dispatch({ type: NOTIFY_TYPES.NOTIFY, payload: { isLoading: true } });
        await axios.delete(`http://localhost:5000/api/v1/post/delete/${idPost}`);
